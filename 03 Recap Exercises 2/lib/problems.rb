...
def most_frequent_bigram(str)
  ...
  hash_count.max_by { |key, value| value }.first
end
...
class Array
  ...
  def pair_sum_count(num)
    ...
    (0...self.length).each do |first_ele|
      (first_ele + 1...self.length).each do |last_ele|
        sum_pairs_count += 1 if self[first_ele] + self[last_ele] == num
      end
    end
