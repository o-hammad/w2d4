...
def all_vowel_pairs(words)
  ...
  (0...words.length).each do |start|
    (start + 1...words.length).each do |finish|
      vowels = "aeiou".split("")
      combined_string = words[start] + " " + words[finish]
      
      combined_string.each_char do |char|
        if vowels.include?(char)
          vowels.delete(char)
        end
      end

      result << combined_string if vowels.empty?
    end
  end
